...
/*   Updated: 2022/09/11 12:56:24 by yed-dyb          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cylinder_init(t_cy *cy, t_ambient ambient)
{
	...
	cy->material = materials(cy->rgb, ambient.ratio, cy->shiness, cy->reflective);
	cy->min = 0;
	cy->max = cy->height;
}
...
t_vector	normal_at_cylinder(t_cy cy, t_vector p)
{
	...
	world_n = vector_mult_matrix(obj_n, cy.transposed_matrix, 1);
	world_n.w = 0;
