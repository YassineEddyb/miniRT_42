...
t_RGB lightning(t_ambient am, t_material m, t_light l, t_vector pos, t_vector v, t_vector n, int in_shadow)
{
    t_RGB diffuse;
    t_RGB specular;
    t_RGB eff_color = colour_init(m.color.red * l.rgb.red * am.rgb.red, m.color.green * l.rgb.green * am.rgb.green, m.color.blue * l.rgb.blue * am.rgb.blue);
    t_vector lightv = normalize(vectorSub(l.pos, pos));
    t_RGB ambient = colour_init(eff_color.red * m.ambient, eff_color.green * m.ambient, eff_color.blue * m.ambient);
    ...
    if (light_dot_n < 0 || in_shadow)
    {
        ...
        if (reflect_dot_eye <= 0)
            ...
            specular = colour_init(l.rgb.red * light_specular, l.rgb.green * light_specular, l.rgb.blue * light_specular);
        }
    }
